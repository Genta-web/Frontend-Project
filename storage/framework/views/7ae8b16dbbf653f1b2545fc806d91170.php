...
<?php if(session('success') || session('error') || session('warning')): ?>
    ...
    <div class="popup-overlay" id="sessionPopup">
        <div class="popup-card popup-<?php echo e($status); ?>">
            <div class="popup-icon">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><?php echo $iconSvg; ?></svg>
            </div>
            <h2 class="popup-title"><?php echo e($title); ?>!</h2>
            <p class="popup-message"><?php echo e($message); ?></p>
...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center dashboard-greeting">
        <div>
            <h1 id="greeting-text">Good afternoon, <?php echo e($employee->name ?? 'Guest'); ?>!</h1>
            <p id="dynamic-info">
                <?php if(($leaveStats['pending_requests'] ?? 0) > 0): ?>
                    You have <?php echo e($leaveStats['pending_requests']); ?> leave request<?php echo e($leaveStats['pending_requests'] > 1 ? 's' : ''); ?> pending
                <?php else: ?>
        ...
        <div class="current-time">
            ...
            <span id="live-time"><?php echo e(now()->format('d M Y, h:i A')); ?></span>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-lg-3 mb-4">
            <div class="dashboard-card card-today">
                ...
                <div class="card-today-header">
                    <h5>Today</h5>
                    <span id="attendance-status-badge" class="status-badge <?php echo e($statusClass); ?>"><?php echo e($statusText); ?></span>
                </div>
                <div class="progress-circle" id="attendance-progress">
                    <div class="progress-inner">
                        <span class="progress-percent"><?php echo e($attendanceStats['percentage'] ?? 0); ?>%</span>
                        <span class="progress-label">attendance</span>
                    </div>
                </div>
                <p id="attendance-status-text"><?php echo e($statusMessage); ?></p>
                ...
                <div class="mt-auto">
                    ...
                    <form action="<?php echo e(route('attendance.checkin')); ?>" method="POST" class="d-grid" id="checkin-form">
                        <?php echo csrf_field(); ?>
                        <button type="submit" class="action-btn btn-checkin mb-2" id="checkin-btn" <?php echo e($hasCheckedIn ? 'disabled' : ''); ?>>
                            <i class="fas fa-map-marker-alt me-2"></i>Check In
                        </button>
                    </form>
                    <form action="<?php echo e(route('attendance.checkout')); ?>" method="POST" class="d-grid" id="checkout-form">
                        <?php echo csrf_field(); ?>
                        <button type="submit" class="action-btn btn-checkout" id="checkout-btn" <?php echo e(!$hasCheckedIn || $hasCheckedOut ? 'disabled' : ''); ?>>
        ...
        <div class="col-lg-3 mb-4">
            <div class="stat-card mb-4">
                <a href="<?php echo e(route('worklogs.index')); ?>">
                    <i class="far fa-folder-open fa-2x text-warning mb-3"></i>
                    <div class="stat-card-title">Total Work Logs</div>
                    <div class="stat-card-value" id="total-work-logs"><?php echo e($stats['total_work_logs'] ?? 0); ?></div>
                </a>
            </div>
            <div class="stat-card mb-4">
                <a href="<?php echo e(route('attendance.index')); ?>">
                    ...
                    <div class="stat-card-value" id="attendance-percentage"><?php echo e($attendanceStats['percentage'] ?? 0); ?>%</div>
                </a>
            </div>
            <div class="stat-card">
                <i class="far fa-calendar-times fa-2x mb-3" style="color: #c97c7b;"></i>
                <div class="stat-card-title">Pending Leaves</div>
                <div class="stat-card-value" id="pending-leaves"><?php echo e($leaveStats['pending_requests'] ?? 0); ?></div>
                <a href="<?php echo e(route('leave.index')); ?>" class="btn-request-leave">View Leaves</a>
            </div>
        </div>
        ...
        <div class="col-lg-6 mb-4">
            <div class="dashboard-card work-log-preview" style="background: #f7fbff; border-radius: 16px; box-shadow: 0 4px 16px rgba(74,144,226,0.07);">
                <div class="work-log-preview-header d-flex justify-content-between align-items-center mb-3 pb-2" style="border-bottom: 1px solid #e8edf2;">
                    ...
                    <a href="<?php echo e(route('worklogs.index')); ?>" class="btn btn-sm btn-outline-primary" style="border-radius:8px;font-weight:600;">View All</a>
                </div>

                <?php if($recentWorkLogs->count() > 0): ?>
                    <div class="mb-4">
                        ...
                        <ul class="task-summary" style="padding:0;margin:0;list-style:none;">
                            <?php $__currentLoopData = $recentWorkLogs->take(3); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $workLog): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="task-item" style="background:#fff;border-radius:10px;padding:1rem 1.2rem;margin-bottom:1rem;box-shadow:0 2px 8px rgba(74,144,226,0.05);border:1px solid #e8edf2;display:flex;justify-content:space-between;align-items:center;">
                                    <div style="flex:1;min-width:0;">
                                        <div style="font-weight:600;font-size:1rem;color:#333A45;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?php echo e(Str::limit($workLog->task_summary, 40)); ?></div>
                                        <div style="font-size:0.95rem;color:#7A869A;margin-top:2px;">
                                            <i class="far fa-calendar-alt me-1"></i><?php echo e($workLog->work_date->format('M j, Y')); ?>

                                        </div>
                                    </div>
                                    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:0.3rem;">
                                        <span class="badge bg-primary" style="font-size:0.95rem;padding:0.5em 1em;border-radius:8px;"><?php echo e($workLog->start_time); ?> - <?php echo e($workLog->end_time); ?></span>
                                    </div>
                                </li>
                    ...
                    <div class="mt-auto d-flex align-items-center justify-content-between pt-2" style="border-top:1px solid #e8edf2;">
                        ...
                        <span class="status-badge on-progress" style="font-size:1rem;padding:0.5em 1.2em;"><?php echo e($stats['work_logs_this_month'] ?? 0); ?> Work Logs</span>
                    </div>
                <?php else: ?>
                    <div class="text-center py-4">
                        <i class="fas fa-clipboard-list fa-3x text-muted mb-3"></i>
                        <p class="text-muted">No work logs yet</p>
                        <a href="<?php echo e(route('worklogs.create')); ?>" class="btn btn-primary btn-sm">Create First Work Log</a>
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const employeeName = "<?php echo e($employee->name ?? 'Guest'); ?>";
    ...
    const attendanceToday = <?php echo json_encode($attendanceToday ?? null, 15, 512) ?>;
    ...
    function updateProgressCircle() {
        ...
        const percentage = <?php echo e($attendanceStats['percentage'] ?? 0); ?>;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Dimas\Desktop\Integrasi KP\backoffice-fasya\resources\views/dashboard/employee.blade.php ENDPATH**/ ?>
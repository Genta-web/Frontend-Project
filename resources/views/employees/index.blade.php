...
{{-- 1. HTML & CSS untuk Pop-up Notifikasi --}}
@if (session('success'))
    ...
    <div class="dashboard-header">
        <div class="row align-items-center">
            ...
            <div class="col-lg-4 text-lg-end">
                <a href="{{ route('employees.create') }}" class="btn comprehensive-btn">
...
<div class="filter-card">
    <form method="GET" action="{{ route('employees.index') }}">
        <div class="filter-group" style="flex-grow: 1.5; min-width: 200px; max-width: 300px;">
            ...
            <input type="text" id="filter_search" name="search" class="form-control" placeholder="Search by name, position..." value="{{ request('search') }}">
        </div>
    ...
    <div class="employee-section">
        <div class="swiper employee-swiper">
            <div class="swiper-wrapper">
                @foreach($employees->chunk(15) as $chunk)
                                ...
                                <div class="employee-card h-100">
                                    <div class="card-main-info">
                                        <div class="avatar" style="width:60px; height:60px;">
                                            ...
                                            @else
                                                <img src="https://ui-avatars.com/api/?name={{ urlencode($employee->name ?? 'Employee') }}&color=7F9CF5&background=EBF4FF"
                                                    alt="{{ $employee->name }}"
                                                    class="rounded-circle img-fluid"
                                                    style="width: 60px; height: 60px; object-fit: cover;">
                                            @endif
                                    ...
                                    <div class="card-tags">
                                        ...
                                        @endphp
                                        <span class="tag" style="{{ $deptColors[$employee->department] ?? 'background-color:#e5e5e5;' }}">
                                            {{ $employee->department }}
                                        </span>
                                        <span class="tag" style="{{ $roleColors[$employee->status === 'active' ? 'Management' : 'Non-Management'] ?? 'background-color:#e5e5e5;' }}">
                                            {{ $employee->status === 'active' ? 'Management' : 'Non-Management' }}
                                        </span>
                                    </div>
                                    <div class="card-meta">
                                        <p>Emp Code: {{ $employee->employee_code }}</p>
                                        <p>Joining Date: {{ \Carbon\Carbon::parse($employee->hire_date)->format('d-M-Y') }}</p>
                                        <div class="d-flex align-items-center mt-3 px-2" style="gap:0;">
                                            <div class="d-flex gap-4">
                                                <a href="{{ route('employees.show', $employee->id) }}" class="btn btn-sm btn-outline-info d-flex align-items-center gap-1">
                                                    <i class="fas fa-eye"></i>
                                                    <span>View</span>
                                                </a>
                                                <a href="{{ route('employees.edit', $employee->id) }}" class="btn btn-sm btn-outline-primary d-flex align-items-center gap-1">
                                                    <i class="fas fa-edit"></i>
                                                    <span>Edit</span>
                                                </a>
                                            </div>
                                            <div style="flex:0 0 auto; margin-left:auto;">
                                                <form action="{{ route('employees.destroy', $employee->id) }}" method="POST" class="delete-form" data-employee-name="{{ $employee->name }}" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                ...
                <div class="pagination-info d-flex align-items-center gap-3">
                    ...
                    <div class="d-flex align-items-center gap-1">
                        ...
                        <input type="number"
                               ...
                               value="{{ $employees->currentPage() }}"
                               onkeypress="if(event.key==='Enter') goToPage(this.value)">
                        <small class="text-muted">of {{ $employees->lastPage() }}</small>
                    </div>
                    @endif
                </div>
            </div>
    ...
    <div class="mt-4 pt-3 border-top">
        <small class="text-muted">
            Total: {{ $employees->count() }} employee{{ $employees->count() != 1 ? 's' : '' }}
        </small>
    </div>
    @endif
...
@push('scripts')
{{-- 3. JavaScript untuk menutup pop-up --}}
<script>
...
function goToPage(page) {
    ...
    const maxPage = {{ $employees->lastPage() ?? 1 }};

    // Validate page number
    if (isNaN(pageNum) || pageNum < 1 || pageNum > maxPage) {
        alert(`Please enter a valid page number between 1 and ${maxPage}`);
        document.getElementById('pageJump').value = {{ $employees->currentPage() ?? 1 }};
        return;
    }
...
document.addEventListener('keydown', function(e) {
    ...
    const currentPage = {{ $employees->currentPage() ?? 1 }};
    const lastPage = {{ $employees->lastPage() ?? 1 }};

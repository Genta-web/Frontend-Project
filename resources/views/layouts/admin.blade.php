...
<nav class="navbar navbar-expand-lg" style="background-color: #87CEEB;">
    <div class="container-fluid">
        <a class="navbar-brand text-white fw-bold" href="{{ route('dashboard') }}">
            <i class="fas fa-users me-2"></i>{{ config('app.name', 'Employee Management System') }}
        </a>
            ...
            <ul class="navbar-nav ms-auto">
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle text-white" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown">
                        <i class="fas fa-user me-1"></i>{{ Auth::user()->display_name ?? Auth::user()->username }}
                        <span class="badge bg-white text-dark ms-1">{{ ucfirst(Auth::user()->role) }}</span>
                    </a>
    ...
    <div class="position-sticky pt-3">
        <ul class="nav flex-column">
            ...
            @if(!Auth::user()->hasRole('system'))
                <li class="nav-item mb-1">
                    <a class="nav-link {{ request()->routeIs('worklogs.*') ? 'active' : '' }}" href="{{ route('worklogs.index') }}">
            ...
            @if(!Auth::user()->hasRole('system'))
                <li class="nav-item mb-1">
                    <a class="nav-link {{ request()->routeIs('dokumen.*') || request()->routeIs('management-document') ? 'active' : '' }}" href="{{ route('dokumen.index') }}">
            ...
            <!--@if(!Auth::user()->hasRole('system'))
                ...
                <li class="nav-item mb-1">
                    <a class="nav-link {{ request()->routeIs('job-batches.schedule') ? 'active' : '' }}" href="{{ route('job-batches.schedule') }}">
            ...
            <!--@if(Auth::user()->hasRole(['admin', 'hr', 'manager']))
                ...
                <li class="nav-item mb-1">
                    <a class="nav-link {{ request()->routeIs('job-batches.index') || request()->routeIs('job-batches.create') || request()->routeIs('job-batches.edit') || request()->routeIs('job-batches.show') ? 'active' : '' }}" href="{{ route('job-batches.index') }}">

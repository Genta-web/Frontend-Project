...
<div class="work-logs-container">
        ...
        <div class="page-header-card">
            <div class="row align-items-center">
                ...
                <div class="col-lg-4 text-lg-end">
                    <a href="{{ route('worklogs.create') }}" class="btn-clean primary" style="border-radius: 50px;">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('worklogs.quickadd') }}" method="POST">
                @csrf
                <div class="row">
                    <div class="col-md-2">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('worklogs.index') }}">
                ...
                <div class="row mt-2">
                    <div class="col-12">
                        <a href="{{ route('worklogs.index') }}" class="btn btn-secondary btn-sm">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        @forelse($workLogs as $index => $workLog)
                            <tr>
                                <td>{{ $workLogs->firstItem() + $index }}</td>
                                @if(!Auth::user()->hasRole('employee'))
                                    <td>
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <strong>{{ $workLog->employee->name ?? 'N/A' }}</strong><br>
                                                <small class="text-muted">{{ $workLog->employee->employee_code ?? 'N/A' }}</small>
                                            </div>
                                        </div>
                                    </td>
                                @endif
                                <td>{{ $workLog->work_date ? $workLog->work_date->format('d M Y') : 'N/A' }}</td>
                                <td>
                                    <small>
                                        <strong>Start:</strong> {{ $workLog->start_time ? \Carbon\Carbon::parse($workLog->start_time)->format('H:i') : '-' }}<br>
                                        <strong>End:</strong> {{ $workLog->end_time ? \Carbon\Carbon::parse($workLog->end_time)->format('H:i') : '-' }}
                                    </small>
                                </td>
                                <td>
                                    <span class="badge {{ $workLog->status_badge_class }}">
                                        {{ $workLog->status_display }}
                                    </span>
                                    @if($workLog->status_updated_at)
                                        <br><small class="text-muted">
                                            Updated: {{ $workLog->status_updated_at->format('d/m H:i') }}
                                        </small>
                                    @endif
                                    @if($workLog->status_update_image)
                                ...
                                <td>
                                    <span title="{{ $workLog->task_summary }}">
                                        {{ Str::limit($workLog->task_summary, 50) }}
                                    </span>
                                </td>
                                <td>
                                    @if($workLog->attachment_image)
                                        <a href="{{ asset('uploads/worklogs/' . $workLog->attachment_image) }}"
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <button type="button" class="btn btn-sm btn-outline-info"
                                                onclick="showDetailModal({{ $workLog->id }}, '{{ $workLog->employee->name ?? 'N/A' }}', '{{ $workLog->work_date ? $workLog->work_date->format('d M Y') : 'N/A' }}', '{{ addslashes($workLog->task_summary) }}', '{{ addslashes($workLog->action_details ?? '') }}', '{{ $workLog->status_display }}', '{{ $workLog->status_update_image ?? '' }}', '{{ $workLog->status_updated_at ? $workLog->status_updated_at->format('d M Y H:i:s') : '' }}')"
                                                title="View Details">
                                            <i class="fas fa-eye"></i>
                                        </button>

                                        @if(Auth::user()->hasRole(['admin', 'hr', 'manager']) || $workLog->employee_id == Auth::user()->employee?->id)
                                            @if(!$workLog->isDone())
                                                <button type="button" class="btn btn-sm btn-outline-success"
                                                        onclick="showUpdateStatusModal({{ $workLog->id }}, '{{ $workLog->status }}', '{{ $workLog->action_details ?? '' }}')"
                                                        title="Update Status">
                                            ...
                                            <button type="button" class="btn btn-sm btn-outline-danger"
                                                    onclick="showDeleteConfirmation('{{ $workLog->employee->name ?? 'Work Log' }} - {{ $workLog->work_date ? $workLog->work_date->format('d M Y') : 'N/A' }}', '{{ route('worklogs.destroy', $workLog) }}')"
                                                    title="Delete">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ Auth::user()->hasRole('employee') ? '7' : '8' }}" class="text-center py-4">

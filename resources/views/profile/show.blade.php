            ...
            <div class="card profile-card">
                ...
                <form id="photoUploadForm" action="{{ route('profile.update-photo') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="profile-avatar-wrapper" onclick="document.getElementById('profilePhotoInput').click();">
                        <img src="{{ route('profile.photo', $user->id) }}" alt="Profile Photo" class="profile-avatar" data-user-id="{{ $user->id }}">
                    ...
                    {{-- Tombol submit tersembunyi, form disubmit via JS --}}
                ...
                <div class="mt-4">
                    ...
                    <div class="info-grid">
                        <dl><dt>Employee Code</dt><dd>{{ $employee->employee_code ?? 'N/A' }}</dd></dl>
                        <dl><dt>Username</dt><dd>{{ $user->username }}</dd></dl>
                        <dl><dt>Full Name</dt><dd>{{ $employee->name ?? 'N/A' }}</dd></dl>
                        <dl><dt>Email Address</dt><dd>{{ $employee->email ?? 'N/A' }}</dd></dl>
                        <dl><dt>Phone</dt><dd>{{ $employee->phone ?? 'N/A' }}</dd></dl>
                        <dl><dt>Hire Date</dt><dd>{{ $employee->hire_date ? $employee->hire_date->format('d M Y') : 'N/A' }}</dd></dl>
                        <dl><dt>Last Login</dt><dd>{{ $user->last_login ? $user->last_login->format('d M Y, H:i') : 'Never' }}</dd></dl>
                        <dl><dt>Role</dt><dd><span class="badge bg-primary">{{ ucfirst($user->role) }}</span></dd></dl>
                        <dl><dt>Employee Status</dt><dd><span class="badge bg-{{ $employee->status === 'active' ? 'success' : 'secondary' }}">{{ ucfirst($employee->status ?? 'N/A') }}</span></dd></dl>
                        <dl><dt>Account Status</dt><dd><span class="badge bg-{{ $user->is_active ? 'success' : 'danger' }}">{{ $user->is_active ? 'Active' : 'Inactive' }}</span></dd></dl>
                    </div>
                </div>

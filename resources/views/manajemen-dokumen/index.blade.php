...
{{-- 1. HTML untuk Pop-up Notifikasi (Sukses & Peringatan Hapus) --}}
@if (session('success'))
...
{{-- 2. CSS untuk Pop-up dan Tampilan Halaman --}}
<style>
/* --- Popup --- */
.popup-overlay {
...
.btn-reset-custom:hover, .btn-reset-custom:focus {
    ...
    box-shadow: 0 4px 15px rgba(3, 169, 244, 0.13);
}
            ...
            <div class="card-body">
                <form method="GET" action="{{ route('dokumen.index') }}" class="row g-3 align-items-end">
                    <div class="col-md-4">
                        ...
                        <input type="date" id="upload_date" name="upload_date" class="form-control" value="{{ request('upload_date') }}">
                    </div>
                    <div class="col-md-4">
                        <label for="uploader" class="form-label fw-bold">Uploader</label>
                        <input type="text" id="uploader" name="uploader" class="form-control" placeholder="Enter uploader username" value="{{ request('uploader') }}">
                    </div>
                    <div class="col-md-4 d-flex align-items-end">
                        <button type="submit" class="btn btn-primary btn-custom me-2">
                            <i class="fas fa-search me-1"></i>Filter
                        </button>
                        <a href="{{ route('dokumen.index') }}" class="btn btn-outline-info btn-reset-custom">
                            Reset
                        </a>
                    </div>
                </form>
                                ...
                                <tr>
                                            ...
                                            <div style="word-wrap: break-word; word-break: break-word; white-space: normal; max-width: 100%;">
                                                ...
                                                @if($item->original_name)
                                                    <small class="text-muted" style="word-wrap: break-word; word-break: break-all; white-space: normal; display: block; margin-top: 4px;">
                                                        <i class="fas fa-file me-1"></i>{{ $item->original_name }}
                                                        @if($item->file_size)
                                                            <span class="ms-2">
                                                                <i class="fas fa-weight me-1"></i>{{ $item->formatted_file_size }}
                                                            </span>
                                                        @endif
                                                    </small>
                                                @else
                                                    <small class="text-muted" style="word-wrap: break-word; word-break: break-all; white-space: normal; display: block; margin-top: 4px;">
                                                        <i class="fas fa-file me-1"></i>{{ basename($item->path) }}
                                                    </small>
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="d-flex align-items-start">
                                            ...
                                            <div style="word-wrap: break-word; word-break: break-word; white-space: normal; max-width: 100%;">
                                                <div class="fw-semibold" style="word-wrap: break-word; word-break: break-word; white-space: normal;">
                                                    {{ $item->uploader ? $item->uploader->username : 'Unknown' }}
                                                </div>
                                                @if($item->uploader)
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group" aria-label="Document Actions">
                                            <a href="{{ route('dokumen.show', $item->id) }}" class="btn btn-sm btn-outline-info" data-bs-toggle="tooltip" title="View Details">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('dokumen.download', $item->id) }}" class="btn btn-sm btn-outline-success" data-bs-toggle="tooltip" title="Download">
                                                <i class="fas fa-download"></i>
                                            </a>
                                            <a href="{{ route('dokumen.edit', $item->id) }}" class="btn btn-sm btn-outline-warning" data-bs-toggle="tooltip" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <button type="button" class="btn btn-sm btn-outline-danger" onclick="confirmDelete({{ $item->id }}, '{{ addslashes($item->title) }}')" data-bs-toggle="tooltip" title="Delete">
            ...
            <form action="{{ route('dokumen.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                     <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="title" name="title" value="{{ old('title') }}" required placeholder="e.g., Q3 Financial Report">
                     ...
                     <div class="mb-3">
                        ...
                        @error('path')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                    </div>
                     <div class="mb-3">
                        <label for="description" class="form-label fw-bold">Description</label>
                        <textarea class="form-control" id="description" name="description" rows="4" placeholder="Enter a brief description (optional)">{{ old('description') }}</textarea>
...
{{-- 4. JavaScript untuk Pop-up & Fungsionalitas Halaman --}}
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.confirmDelete = function(id, title) {
        ...
        deleteForm.action = `{{ url('management-document') }}/${id}`;

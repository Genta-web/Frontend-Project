...
<div class="leave-container">
    ...
    <div>
        <a href="{{ route('leave.create') }}" class="btn comprehensive-btn">
            <i class="fas fa-plus me-2"></i> New Leave Request
        </a>
    </div>

    {{-- Logika PHP untuk menghitung status cuti tidak memengaruhi tampilan,
         jadi tidak disertakan di sini tetapi bisa Anda letakkan di mana saja di dalam controller atau view--}}
    @if(!Auth::user()->isEmployee())
        @php
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        @forelse($leaves as $index => $leave)
                            <tr class="leave-row
                                @if($leave->status === 'pending') table-info
                                @elseif($leave->status === 'waiting') table-warning
                                @elseif($leave->status === 'approved') table-success
                                @elseif($leave->status === 'rejected') table-danger
                                @endif
                            " data-status="{{ $leave->status }}" data-leave-id="{{ $leave->id }}">
                                @if(Auth::user()->hasRole(['admin', 'hr']))
                                    <!-- Checkbox for bulk selection -->
                                    <td class="text-center align-middle">
                                        <div class="form-check">
                                            <input class="form-check-input leave-checkbox" type="checkbox" value="{{ $leave->id }}" id="leave_{{ $leave->id }}" onchange="updateBulkActions()">
                                            <label class="form-check-label" for="leave_{{ $leave->id }}"></label>
                                        </div>
                                    </td>
                                @endif
                                    ...
                                    <td class="align-middle">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="employee-details">
                                                <h6 class="mb-1 fw-semibold">{{ $leave->employee->name ?? 'Unknown' }}</h6>
                                ...
                                <td class="align-middle">
                                    <div class="date-range-container">
                                        <div class="d-flex align-items-center mb-1">
                                            ...
                                            <strong class="text-dark">{{ $leave->start_date->format('d M Y') }}</strong>
                                        </div>
                                        <div class="d-flex align-items-center">
                                        ...
                                        @elseif($leave->status === 'approved')
                                            ...
                                            <div class="mt-1">
                                                <small class="text-success">
                                                    ...
                                                    by {{ $leave->approvedBy->username ?? 'Admin' }}
                                                </small>
                                            </div>
                                ...
                                <td class="text-center align-middle">
                                    <div class="action-buttons">
                                        ...
                                        @if(in_array($leave->status, ['pending', 'waiting']) && Auth::user()->hasRole(['admin', 'hr', 'manager']))
                                            @if($leave->status === 'waiting')
                                                ...
                                                <div class="urgent-actions">
                                                    ...
                                                    <button type="button"
                                                            ...
                                                            onclick="approveLeaveRequest({{ $leave->id }}, '{{ $leave->employee->name ?? 'Employee' }}', '{{ $leave->leave_type_display }}', '{{ $leave->start_date->format('d M Y') }}', '{{ $leave->end_date->format('d M Y') }}', {{ $leave->total_days }})"
                                                    ...
                                                    <button type="button"
                                                            ...
                                                            onclick="rejectLeaveRequest({{ $leave->id }}, '{{ $leave->employee->name ?? 'Employee' }}', '{{ $leave->leave_type_display }}', '{{ $leave->start_date->format('d M Y') }}', '{{ $leave->end_date->format('d M Y') }}', {{ $leave->total_days }})"
                                        ...
                                        @elseif($leave->status === 'pending' && $leave->employee_id == Auth::user()->employee?->id)
                                            ...
                                            <button type="button"
                                                    ...
                                                    onclick="showDeleteConfirmModal({{ $leave->id }}, '{{ $leave->employee->name }}', '{{ $leave->leave_type }}', '{{ $leave->start_date }}', '{{ $leave->end_date }}')">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ Auth::user()->hasRole('employee') ? '6' : (Auth::user()->hasRole(['admin', 'hr']) ? '8' : '7') }}" class="text-center py-5">
                        ...
                        <div class="d-flex justify-content-md-end justify-content-center mt-2 mt-md-0">
                            <nav aria-label="Leave requests pagination">
                                <div class="pagination-wrapper">
                                    {{ $leaves->links('custom.pagination') }}
                                </div>
                            </nav>
                        </div>
                    </div>
